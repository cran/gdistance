...
An important advantage of using conductance is that it makes it possible to store values in computer memory very efficiently, using so-called sparse matrices.
...
The function \code{geoCorrection} corrects this distortion by multiplying the 
...
With a more complex formula, we can approximate the non-overlapping part of the trajectory (Figure~\ref{fig:six}). This is done in the following code.
\vspace{-1.0em}
<<gdistance-15>>=
